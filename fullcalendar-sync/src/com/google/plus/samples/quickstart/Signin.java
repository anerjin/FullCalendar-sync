...
public class Signin {
   ...
   */
  private static final String APPLICATION_NAME = "Accompani- FullCalendar Sync";
  ...
  public static void main(String[] args) throws Exception {
    ...
    servletHandler.addServletWithMapping(CalendarServlet.class, "/calendar");
  ...
  public static class CalendarServlet extends HttpServlet 
  {
        ...
        throws ServletException, IOException 
    {
      ...
      try {
        ...
        String content = new Scanner(new File("calendar_template.html")).useDelimiter("\\Z").next();
      ...
      String allJson = "[\n";
       ...
       /// Get all events from the primary calendar
        ...
        do {
          Events events = calService.events().list("primary").setPageToken(pageToken).execute();
          List<Event> items = events.getItems();
          for (Event event : items) {
            
            String json = "{\n";

          //  if (event.getId()!= null)        json += String.format("   id: %s,\n", event.getId());
            if (event.getSummary() != null)  json += String.format("   title: '%s',\n", event.getSummary());

            if (event.getStart() != null){
                 String parsedDate = event.getStart().getDateTime().toString();
                json += String.format("   start: '%s',\n", parsedDate.substring(0,parsedDate.indexOf('.')));
            }

            if (event.getEnd() != null)      {
               String parsedDate = event.getStart().getDateTime().toString();
              json += String.format("   end: '%s',\n", parsedDate.substring(0,parsedDate.indexOf('.')));
            }

            if (event.getHtmlLink()!= null)  json += String.format("   url: '%s'\n", event.getHtmlLink());
          //  if (event.getColorId() != null)  json += String.format("   color: '%s'\n", event.getColorId());

            json+= "\n},\n";
            if (event.getStart()!= null && event.getSummary()!=null)
              allJson += json;
          }
        ...
        allJson = content.replaceAll("_REPLACE_", allJson);
    ...
    }   // end of doGet function


  } /// End of Calendar servlet
}
